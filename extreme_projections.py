...
for var in ['tas','SPEI','rx5']:
	for file in GHA['support']['files'][var]:
		# interprete files
		print file
		if file.split('_')[-3][0]=='r':
			rcp=file.split('/')[-2]
			model=file.split('_')[-4]
		if file.split('_')[-4][0]=='r':
			rcp=file.split('/')[-2]
...
if False:
	...
	for rcp in range(2):
		for period in ['2030s','2040s']:
			Z=GHA['SPEI']['exposure_diff'][rcp_str[rcp]]['ensemble_mean'][period]*100
			Z[Z==0]=np.nan
...
if False:
	...
	for mon in range(3):
		for model in GHA['support']['model_names']:
			Z=GHA[var]['warm_season'][model][mon,:,:]
			Z[Z==0]=np.nan
...
if False:
	...
	count=0
	for model in GHA['support']['model_names']:
		Z=GHA[var]['threshold'][model]-273.15
		Z[Z==0]=np.nan
...
if False:
	for rcp in ['RCP2.6','RCP8.5']:
		...
		for period in ['ref','2030s','2040s']:
			for model in GHA['support']['model_names']:
				Z=GHA['tas']['exposure'][rcp]['models'][model][period]
				Z[Z==0]=np.nan
...
if False:
	...
	for rcp in range(2):
		for period in ['2030s','2040s']:
			Z=GHA['tas']['exposure_diff'][rcp_str[rcp]]['ensemble_mean'][period]*100
			Z[Z==0]=np.nan
			print rcp_str[rcp],period,np.mean(np.ma.masked_invalid(Z)),np.min(np.ma.masked_invalid(Z)),np.max(np.ma.masked_invalid(Z))
...
if False:
	...
	for mon in range(3):
		for model in GHA['support']['model_names']:
			Z=GHA[var]['wet_season'][model][mon,:,:]
			Z[Z==0]=np.nan
	...
	cb.locator = tick_locator
	cb.update_ticks()
	plt.savefig('/Users/peterpfleiderer/Documents/Projects/WB_DRM/plots/'+iso+'/'+iso+'_'+var+'_season.png')
	plt.clf()
		...
		for x in range(GHA[var]['climatology'][model].shape[2]):
			if np.isfinite(GHA[var]['wet_season'][model][0,y,x]):
				relevant_time_indices=np.where((GHA['support']['year']>start_year) & (GHA['support']['year']<=start_year+20) \
					...
					& (GHA[var]['RCP2.6']['models'][model][:,y,x]<1000)\
					& (np.isfinite(GHA[var]['RCP2.6']['models'][model][:,y,x])))[0]
...
if False:
	...
	cb.locator = tick_locator
	cb.update_ticks()
	plt.savefig('/Users/peterpfleiderer/Documents/Projects/WB_DRM/plots/'+iso+'/'+iso+'_'+var+'_thresh.png')
	plt.clf()
...
if True:
	for rcp in ['RCP2.6','RCP8.5']:
		...
		for period in ['ref','2030s','2040s']:
			for model in GHA['support']['model_names']:
				Z=GHA[var]['exposure'][rcp]['models'][model][period]
				Z[Z==0]=np.nan
		...
		cb.locator = tick_locator
		cb.update_ticks()
		plt.savefig('/Users/peterpfleiderer/Documents/Projects/WB_DRM/plots/'+iso+'/'+iso+'_'+var+'_exposure_'+rcp+'.png')
		plt.clf()
...
if True:
	...
	for rcp in range(2):
		for period in ['2030s','2040s']:
			Z=GHA[var]['exposure_diff'][rcp_str[rcp]]['ensemble_mean'][period]*100
			Z[Z==0]=np.nan
			print rcp_str[rcp],period,np.mean(np.ma.masked_invalid(Z)),np.min(np.ma.masked_invalid(Z)),np.max(np.ma.masked_invalid(Z))
	...
	cb.update_ticks()
	fig.tight_layout()
	plt.savefig('/Users/peterpfleiderer/Documents/Projects/WB_DRM/plots/'+iso+'/'+iso+'_'+var+'_exposure_diff.png')
